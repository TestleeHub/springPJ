 ...
 */
 let submit_check = function() {
	...
	//이메일 email2가없으면 ng , ,com이 없어도 ng
	if(!document.userInfoForm.email1.value || !document.userInfoForm.email2.value){
		alert("이메일을 입력하세요");
		document.userInfoForm.userEmail1.focus();
		return false;
	}
	else if(!reg_email.test(document.userInfoForm.email1.value + "@" + document.userInfoForm.email2.value)) {
		alert("이메일 확인 하세요");
		document.userInfoForm.email1.value = "";
		document.userInfoForm.email2.value = "";
		document.userInfoForm.email3.value = "0";
		document.userInfoForm.email1.focus();
		return false;
	}
}
