			...
			<div>
				<center>
					...
					<c:if test="${paging.startPage > 10}">
						<a href="${path}/productlistCategory.cu?pageNum=${paging.prev}&category=${category}">[이전]</a>
					</c:if>
					<!-- 페이지 번호 처리 -->
					<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
						<a href="${path}/productlistCategory.cu?pageNum=${num}&category=${category}">${num}</a>
					</c:forEach>
					<!-- 다음 버튼 활성화 여부 -->
					<c:if test="${paging.endPage < paging.pageCount}">
						<a href="${path}/productlistCategory.cu?pageNum=${paging.next}&category=${category}">[다음]</a>
					</c:if>
				</center>
			</div>
		</div>
