...
<title>찜 목록 ${totalProductCnt}개의 서비스</title>
...
<div class="container-fluid">
			<div class="row">
				...
				<div class="col-md-10">
					<h3>찜 목록 ${totalProductCnt}개의 서비스</h3>
				</div>
				<div class="col-md-1"></div>
			</div>
			<div class="row">
			...
			<div>
				<center>
					...
					<c:if test="${paging.startPage > 10}">
						<a href="${path}/productlist.cu?pageNum=${paging.prev}">[이전]</a>
					</c:if>
					<!-- 페이지 번호 처리 -->
					<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
						<a href="${path}/productlist.cu?pageNum=${num}">${num}</a>
					</c:forEach>
					<!-- 다음 버튼 활성화 여부 -->
					<c:if test="${paging.endPage < paging.pageCount}">
						<a href="${path}/productlist.cu?pageNum=${paging.next}">[다음]</a>
					</c:if>
				</center>
			</div>
		</div>
